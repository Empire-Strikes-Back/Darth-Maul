...
(comment
  ...
  (let [port (or (System/getenv "Darth_Maul_IPFS_PORT") "5001")
    ...
    (Darth-Maul.corn/subscribe-process
     {:sub| sub|
      ...
      (when-let [value (<! ops|)]
        (condp = (:op value)
          ...
          (let [{:keys [frequency role]} value
                id| (chan 1)
                port (or (System/getenv "Darth_Maul_IPFS_PORT") "5001")
                ipfs-api-url (format "http://127.0.0.1:%s" port)
                games-topic (Darth-Maul.corn/encode-base64url-u "raisins")
                game-topic (Darth-Maul.corn/encode-base64url-u frequency)
                _ (Darth-Maul.corn/subscribe-process
                   {:sub| sub|
                    :cancel| cancel-sub|
                    :frequency frequency
                    :ipfs-api-url ipfs-api-url
          ...
          (let [discover-jframe (JFrame. "discover")]
            (Darth-Maul.persimmons/discover-process
          ...
          (let [settings-jframe (JFrame. "settings")]
            (Darth-Maul.persimmons/settings-process
