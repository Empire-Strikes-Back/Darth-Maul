...
(defn create-ui
  ...
  (let [jtab-panel (JPanel.)
    ...
    (go
      (loop []
        (when-let [value (<! draw|)]
          (Darth-Maul.seed/invoke-later-on-swing-edt
           (fn [_]
             (let []
...
(defn process
  ...
  (let []
    ...
    (let [ipfs (IPFS. "/ip4/127.0.0.1/tcp/5001")
          ...
          topic (Darth-Maul.ipfs-http/encode-base64url-u "raisins")
          id (-> ipfs (.id) (.get "ID"))
          sub| (Darth-Maul.ipfs-http/pubsub-sub base-url  topic)]
        ...
        (loop []
          (when-let [value (<! sub|)]
            (when-not (= (:from value) id)
              (println (merge value
                              {:data (-> (:data value) (Darth-Maul.ipfs-http/decode-base64url-u) (read-string))})))
            (recur))))

      (go
        (loop []
          (<! (timeout 2000))
          (Darth-Maul.ipfs-http/pubsub-pub base-url topic (str {:id id
...
(comment
  ...
  (go
    (loop []
      (<! (timeout 2000))
      (-> ipfs (.-pubsub) (.pub topic (Darth-Maul.ipfs-http/encode-base64url-u (format "%s %s" (rand-int 100) id))))
      (recur)))
  ;
  )
